...
const part1Solution = (inputArray) => {
  ...
  const vectorPoints1 = traverse(inputArray[0]);
  const vectorPoints2 = traverse(inputArray[1]);
  const intersectingPoints = getInterSections(vectorPoints1, vectorPoints2);
  const distancesOfPoints = intersectingPoints.map(p => Math.abs(p.x) + Math.abs(p.y)).sort((d1, d2) => d2 - d1)
  
  console.log('VZZZ1', vectorPoints1);
  // console.log('VZZZ2', vectorPoints2);

  return distancesOfPoints;
};

console.log('solution 1 answer: ', part1Solution(INPUTS));
